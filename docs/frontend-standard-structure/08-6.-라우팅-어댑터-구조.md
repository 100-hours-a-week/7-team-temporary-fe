...
- Next.js Pages Router와 혼동을 막기 위해 루트 `pages/` 디렉터리는 사용하지 않는다.
- 문서용 샘플이나 설명이 필요하면 `docs/` 아래로 이동한다.
- 부득이하게 유지해야 한다면 `pages (DO_NOT_USE)`로 명시한다.
...
└── src/
    ...
    │       │   └── README.md         # AuthStore 위치/역할 근거 명시
...
- Page는 hook 호출과 props 전달만 수행하며, query/mutation 파라미터 조합은 Page Hook에서 담당한다.
